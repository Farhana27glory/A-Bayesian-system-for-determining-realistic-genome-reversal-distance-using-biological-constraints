...
int main()
{
	...
	for (int i = 0; i < 6; i++)
	{
		...
		{
	        myint2 = findTop(word);
			myfile2 << myint2 <<" times, "<< "Posterior Probability is: "<< double((myint2/50.0) * 100.0) << "%" << "\n" << word << endl;
			myfile3 << myint2 <<" times, "<< "Posterior Probability is: "<< double((myint2/50.0) * 100.0) << "%" << "\n" << word << endl;
			myfile4 << myint2 <<" times, "<< "Posterior Probability is: "<< double((myint2/50.0) * 100.0) << "%" << "\n" << word << endl;
		    myfile5 << myint2 <<" times, "<< "Posterior Probability is: "<< double((myint2/50.0) * 100.0) << "%" << "\n" << word << endl;
		}
